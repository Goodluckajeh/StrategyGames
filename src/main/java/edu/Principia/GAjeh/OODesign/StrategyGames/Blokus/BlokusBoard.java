...
public class BlokusBoard implements Board, Serializable {
	...
	final int STARTING_SPOT_ROW = 4; // 5 in 1-based indexing
	final int STARTING_SPOT_COL = 4; // 5 in 1-based indexing
	final int STARTING_SPOT_ROW2 = 9; // 10 in 1-based indexing
	final int STARTING_SPOT_COL2 = 9; // 10 in 1-based indexing
	private int value;
	private final int row_12 = 12;
	private final int row_13 = 13;
	private final int row_9 = 9;
	private final int squareValue = 50;
	...
	public String toString() {
		...
		for (int i = 0; i < DIM; i++) {
			...
			for (int j = 0; j < DIM; j++) {
				...
				} else if ((i == STARTING_SPOT_ROW && j == STARTING_SPOT_COL)
						|| (i == STARTING_SPOT_ROW2 && j == STARTING_SPOT_COL2)) {
			...
			if (isDiagonallyAdjacent) {
				if (isDirectlyAdjacent) {
					throw new InvalidMoveException(
							"Pieces owned by the same player cannot be directly adjacent to each other");
				}
			} else {
	...
	public int getValue() {
		...
		int xScore = squareValue * countControlledSquares(PLAYER_0) + countValidMoveSquares(PLAYER_0);
		int oScore = squareValue * countControlledSquares(PLAYER_1) + countValidMoveSquares(PLAYER_1);
		value = xScore - oScore;
	...
	private int countValidMoveSquares(int player) {
		...
		// System.out.println("Player " + player + "'s valid moves: " +
		// validMoves.size());
		// print the valid moves for the player

		// Count all the squares from valid moves
		for (Move move : validMoves) {
			// print each move for the player
			if (!((BlokusMove) move).pass) {
				// print the move and the player
				// System.out.println("Player " + player + "'s move: " + move.toString());
				...
				for (Location loc : piece.locations) {
					// print the location of the piece and the player
					squares += (isLocationInBounds(
							new Location(((BlokusMove) move).row + loc.row, ((BlokusMove) move).col + loc.col)) ? 1 : 0);
				}
			}
		}
		// System.out.println("Player " + player + "'s valid move squares: " + squares);
	...
	public List<? extends Move> getValidMoves() {
		...
		if (passCount >= 2 || value == WIN || value == -WIN) {
	...
	public void undoMove() {
		...
		BlokusMove lastMove = moveHistory.remove(moveHistory.size() - 1);
		currentPlayer = -currentPlayer; // Switch the current player back

		if (lastMove.pass) {
			passCount--;
		} else {
			// Retrieve the piece played in the last move
			List<Piece> pieces = currentPlayer == PLAYER_0 ? player1Pieces : player2Pieces;
			...
			if (playedPiece == null) {
				throw new IllegalStateException("Cannot find the last played piece");
			}

			// Mark the piece as not played
			playedPiece.isPlayed = false;

			// Reset the board state
			playedPiece.setOrientation(lastMove.orientation);

			for (Location loc : playedPiece.locations) {
				int actualRow = lastMove.row + loc.row;
				int actualCol = lastMove.col + loc.col;
				if (isWithinBoard(actualRow, actualCol)) {
					if ((actualRow == STARTING_SPOT_ROW && actualCol == STARTING_SPOT_COL) ||
							(actualRow == STARTING_SPOT_ROW2 && actualCol == STARTING_SPOT_COL2)) {
						board[actualRow][actualCol] = 's'; // If it was a starting square
					} else {
			...
			// Reset the piece's orientation to base configuration
	...
	private void resetGameState() {
		if (moveHistory.isEmpty() || passCount < 2) {
			value = calculateValue(); // Recalculate the game value
		}
	}

	private int calculateValue() {
		int xScore = 50 * countControlledSquares(PLAYER_0);
		int oScore = 50 * countControlledSquares(PLAYER_1);
